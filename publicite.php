       ...
       if( isset($_POST['changer']) && $erreur==null && $_POST['vide']=='Non')
       {
        ...
        // url de vidéo 
                if($_FILES['uplaodI']['size']>1)
                {
                 ...
                 // nomer les vidéos par rand 
                $r=rand(5, 10000);
               $extention= strrchr($_FILES['uplaodI']['name'],'.');
               $nomVideo=$r.$extention; 

           
               $lesnomsVideo=glob('publicite/*');  
        
               
              foreach ($lesnomsVideo as $value )
                {
                  if($value=='publicite/'.$nomVideo)                  
                    {
                 ...
                 $urlNomvideo='publicite/'.$nomVideo;
                  
                 move_uploaded_file($_FILES['uplaodI']['tmp_name'], $urlNomvideo); 
                 $url=$urlNomvideo; 
                 }
                 else
                 {
