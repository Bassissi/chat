...
session_start(); 
	...
	if(isset($_POST['submit'] ) && $type=='perdu')
	{
	     ...
	     // vérifier de date pedru ou date trouvé 
	       ...
	       {    
                ...
                // nomer les images par rand 
                     ...
                     $lesnomsImage=glob('img/*');  

              
                    
                    foreach ($lesnomsImage as $value )
                      {
                        if($value=='img/'.$nomImage)                  
                          {
          ...
          /*
	             ...
	             $lesnomsImages=glob('img/*'); 						  

					foreach ($lesnomsImages as $value )
						{
							if($value==$nomDossier)
							{
		...
		*/		 
				 ...
				 // ranger l'adresse si elle écrit par la google map

						function supp_msg_original($message)
					{
					$pos=strpos($message, ", Marseille, France");
					if ($pos == false)
					        {
					        return $message;
					        }
                    ...
                    $rue =supp_msg_original($_POST['rue']); 
	             
	             $annonce=new Annonce('perdu');
			  	$annonce->ecrireAnnonce($_POST['nom'],$_POST['sex'],$url,$_POST['race'],$couleurs,$_POST['poils'],$_POST['pelage'],$_POST['castre'],$datePT,$rue,$_POST['arrondissement'],$description,$_POST['phone'],$dateNaissance);

			  	// partie de Google Map

                $idgoogle=Annonce::chercherId($_POST['phone']);
                if($_POST['latitude']!=null && $_POST['longitude']!=null )
                {
                	Googlemape::ajouter($idgoogle,$_POST['latitude'],$_POST['longitude']); 
                }
		...
		*/		 
             ...
             $annonce=new Annonce('trouve');
			   if($_POST['nom']!=null)
			       ...
			       $nom='chat par défaut';
					  	$annonce->ecrireAnnonce($nom,$_POST['sex'],$url,$_POST['race'],$couleurs,$_POST['poils'],$_POST['pelage'],$_POST['castre'],$datePT,$rue,$_POST['arrondissement'],$description,$_POST['phone'],$dateNaissance);
			    if($_POST['nom']==null)
			            {
